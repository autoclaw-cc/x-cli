...
- **Make the tab render before reading it.** Tabs the daemon opens sit in the background: Chrome reports `visibilityState: "hidden"`, skips layout, and never fires `IntersectionObserver`. Clicks hit 0×0 elements, infinite scroll and lazy images never fire, and the failure looks exactly like the site blocking you. Every `browser/client.go` here now carries `browser/activate.go`; `Navigate` calls `Activate()` for you. Keep that when adding a CLI, and expose the `--activate tab|window|off` flag (`cmd/activate.go`) plus the shared `WEBBRIDGE_ACTIVATE` env var. **Activating is the default** — `browser.DefaultActivateMode = ActivateTab` in every CLI here: the page renders, and the user keeps whatever they had in front of them. `window` (`Page.bringToFront`) also works but measurably makes Chrome the frontmost application, so it stays opt-in — **never take the user's screen by default**. A new CLI that only ever reads the server-rendered first screen may set the constant to `ActivateOff`; say why in a comment, because getting it wrong fails invisibly.
