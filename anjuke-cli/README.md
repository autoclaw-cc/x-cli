...
**Pre-built binary (recommended)**: download `anjuke-cli-{platform}.tar.gz` from [Releases](https://github.com/better-world-ai/x-cli/releases?q=anjuke-cli) and extract. macOS users: clear the Gatekeeper quarantine once with `xattr -d com.apple.quarantine ./anjuke-cli`.
