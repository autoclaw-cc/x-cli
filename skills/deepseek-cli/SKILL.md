---
name: deepseek-cli
description: Use when Codex needs to inspect or ask DeepSeek through the local x-cli DeepSeek browser CLI, including checking login state, listing visible DeepSeek web capabilities, sending a prompt, waiting for a stable answer, or saving the answer locally through an isolated Kimi WebBridge browser session.
---
...
- Use only the isolated DeepSeek Chrome profile connected through Kimi WebBridge, normally `--webbridge-url http://127.0.0.1:10086`.
...
3. Use `chat ask` for normal prompt/answer work. It sends the prompt through Kimi WebBridge page actions, then waits for the newest assistant answer to remain stable before returning.
